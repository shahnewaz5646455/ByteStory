...
export default function PdfConverter() {
  ...
  useEffect(() => {
    if (!window.speechSynthesis) {
      setError(
        "Web Speech API not supported in this browser. Try Chrome, Firefox, or Edge."
      );
      return;
    }
      ...
      for (let i = 1; i <= Math.min(pdf.numPages, maxPages); i++) {
        const page = await Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(
              () => reject(new Error("Page processing timeout")),
              10000
            )
          ),
        ]);
        ...
        extractedText +=
          pageText + (i < Math.min(pdf.numPages, maxPages) ? " " : "");
      }

      setText(extractedText || "No text extracted from the PDF.");

      // Auto-format the text after extraction
      if (extractedText) {
        await handleFormatText(extractedText);
      }
    } catch (err) {
  ...
  const handleFormatText = async (textToFormat = null) => {
    const textContent = textToFormat || text;
    ...
    try {
      ...
      console.error("Error:", err);
      setError(`Error formatting text: ${err.message || "Unknown error"}`);
    } finally {
  ...
  const handleSummarize = async () => {
    ...
    try {
      const textToSummarize = formattedText || text;
      const formData = new FormData();
      formData.append("text", textToSummarize);
  ...
  const handleSpeak = () => {
    if (!summary || !selectedVoice || !window.speechSynthesis) {
      setError(
        "No summary or voice selected. Please generate a summary and choose a voice."
      );
      return;
    }
...
<div className="text-center mb-8">
  <div className="flex items-center justify-center gap-3 mb-4">
    <div className="md:p-3 p-2 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-full">
      <FileText className="md:h-8 md:w-8 h-5 w-5 text-white" />
    </div>
    <h1 className="md:text-4xl text-3xl font-bold text-gray-900 dark:text-white">
      PDF Summarizer
    </h1>
  </div>
  <p className="md:text-lg text-base text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
    Upload a PDF file to extract its text content and generate an AI-powered summary
          ...
          {fileName && (
            <div className="flex items-center justify-between bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-lg mb-4 border border-indigo-100 dark:border-indigo-800/30">
              <div className="flex items-center">
                ...
                <div>
                  <span className="text-indigo-700 dark:text-indigo-300 font-medium truncate max-w-xs block">
                    {fileName}
                  </span>
                  {pageCount > 0 && (
        ...
        {text && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Original Extracted Text Panel */}
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 border border-gray-100 dark:border-gray-700 lg:col-span-1">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 border border-gray-100 dark:border-gray-700 lg:col-span-1">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold text-gray-900 dark:text-white flex items-center">
                  <Wand2 className="w-5 h-5 mr-2 text-green-600 dark:text-green-400" />
                </h2>
                <div className="flex space-x-2">
                  <button
                    onClick={() => handleFormatText()}
                    disabled={isFormatting || !text}
                    className="flex items-center px-2 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:from-emerald-600 hover:to-green-600 transition-all duration-300 shadow-md hover:shadow-lg disabled:opacity-50 font-medium"
                  >
                    {isFormatting ? (
                      <>
                        <RotateCw className="w-2 h-2 mr-2 animate-spin" />
                        Formatting...
                      </>
                    ) : (
                      <>
                        <Wand2 className="w-2 h-2 mr-2" />
                        Reformat
                      </>
                    )}
                  </button>
                  <button
                    ...
                    className={`flex items-center px-4 py-2 rounded-lg transition-all duration-300 font-medium ${
                      formattedCopied
                        ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-800"
                        : "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 hover:bg-green-200 dark:hover:bg-green-900/50 border border-green-200 dark:border-green-800"
                    }`}
                  >
                    {formattedCopied ? (
                      <>
              ...
              <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-green-50 dark:bg-green-900/20 h-96 overflow-y-auto">
                {isFormatting ? (
                  <div className="flex flex-col items-center justify-center h-full">
                    <div className="w-12 h-12 rounded-full animate-spin border-4 border-solid border-green-500 border-t-transparent mb-4"></div>
                    <p className="text-gray-600 dark:text-gray-300">
                      Formatting text with AI...
                    </p>
                  </div>
                ) : formattedText ? (
                  <pre className="whitespace-pre-wrap text-sm text-gray-800 dark:text-gray-200 font-sans leading-relaxed">
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-gray-500 dark:text-gray-400 text-center">
                    <Wand2 className="w-16 h-16 mb-4 text-green-300 dark:text-green-600" />
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 border border-gray-100 dark:border-gray-700 lg:col-span-1">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-4">
                <h2 className="text-xl font-semibold text-gray-900 dark:text-white flex items-center justify-center sm:justify-start">
                ...
                {summary && (
                  <div className="flex flex-wrap justify-center sm:justify-end gap-2">
                    <button
                      ...
                      className={`flex items-center px-3 py-2 rounded-lg transition-all duration-300 font-medium text-sm ${
                        summaryCopied
                          ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 border border-green-200 dark:border-green-800"
                          : "bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-200 dark:hover:bg-indigo-900/50 border border-indigo-200 dark:border-indigo-800"
                      }`}
                    ...
                    <button
                      ...
                      disabled={
                        !summary || !selectedVoice || !window.speechSynthesis
                      }
                    >
                      {isSpeaking ? (
                        <>
                        ...
                        <>
                          ...
                          <span className="text-xs sm:text-sm">Speak</span>
                        </>
                      )}
                    </button>
                    <button
                      onClick={handleStopSpeech}
                      className="flex items-center px-3 py-2 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 rounded-lg hover:bg-red-200 dark:hover:bg-red-900/50 transition-all duration-300 font-medium text-sm border border-red-200 dark:border-red-800"
                      disabled={!isSpeaking}
                      title="Stop Speech"
                    >
              ...
              <div className="mb-4">
                ...
                <select
                  ...
                  className="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 transition-colors text-sm sm:text-base"
              ...
              <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-900/50 h-80 overflow-y-auto">
                {isSummarizing ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-gray-500 dark:text-gray-400 text-center">
                    ...
                    <p className="px-2">
                      Click the Summarize button to generate an AI-powered
                      summary of your PDF content.
                    </p>
                    <button
                      onClick={handleSummarize}
                      disabled={isSummarizing || (!formattedText && !text)}
                      className="mt-4 flex items-center px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-purple-600 hover:to-indigo-600 transition-all duration-300 shadow-md hover:shadow-lg disabled:opacity-50 font-medium text-sm sm:text-base"
                    >
                      {isSummarizing ? (
                        <>
        ...
        <div className="mt-8 text-center text-sm text-indigo-600 dark:text-indigo-400">
          <p>
            Powered by AI • Maximum file size: 10MB • Processes first 5 pages
          </p>
        </div>
      </div>
    </div>
